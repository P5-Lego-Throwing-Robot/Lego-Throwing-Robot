...
void go_to_joint_position(std::vector<double> joint_goal) {
	ROS_INFO("here is data: %f", joint_goal[0]);
ROS_INFO("here is data: %f", joint_goal[1]);
ROS_INFO("here is data: %f", joint_goal[2]);
    moveit::planning_interface::MoveGroupInterface move_group(PLANNING_GROUP);
    //move_group.setEndEffectorLink("tcp");
...
void throw_to(double position[3]) {
	...
	ROS_INFO("here is data: %f", position[0]);
ROS_INFO("here is data: %f", position[1]);
ROS_INFO("here is data: %f", position[2]);

ROS_INFO("here is data: %f", goal_position[0]);
ROS_INFO("here is data: %f", goal_position[1]);
ROS_INFO("here is data: %f", goal_position[2]);
	...
	ROS_INFO("Yeet3a");
ROS_INFO("joint one angle: %f", joint_one_angle);
    go_to_joint_position(joint_position_start);
	ROS_INFO("Yeet3b");
